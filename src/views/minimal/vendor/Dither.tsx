/**
 * Ported to TypeScript from React Bits (MIT), "Dither" background:
 ...
 * app's own styling.
 *
 * This module is the only reason `three`, `@react-three/fiber`,
 * `@react-three/postprocessing` and `postprocessing` are dependencies, which is
 * why DitherBackground loads it lazily.
 */
/* eslint-disable react-hooks/refs, react-hooks/exhaustive-deps --
   react-three-fiber's model is imperative by design: the uniforms object is
...
/**
 * Diverges from upstream, which takes no constructor arguments and relies on a
 * forwardRef wrapper to push props through setters. `wrapEffect` derives a
 * component's props from the effect constructor's first parameter, so taking
 * an options object is what makes `<RetroEffect colorNum pixelSize />` type.
 */
...
export interface DitherProps {
  ...
  /** Linear RGB in 0..1, not a CSS colour. */
  waveColor?: RgbTriplet;
  /** Quantisation levels the Bayer dither snaps to. */
  colorNum?: number;
  pixelSize?: number;
...
export default function Dither({
  ...
  return (
    <Canvas
      ...
      // dpr 1 on purpose: the whole look is a low-resolution dither, so
      // rendering at device pixel ratio costs fill rate and fights the effect.
      dpr={1}
